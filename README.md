# AlgoPOS — Optimized Transaction & Inventory Management System
...
└── src/main/
    ├── java/com/algopos/
    │   ├── model/Product.java
    │   ├── algorithms/InventoryManager.java     <- Binary Search
    │   ├── algorithms/TransactionEngine.java    <- DP Coin-Change
    │   ├── util/CSVLoader.java
    │   ├── util/AppBootstrap.java               <- @WebListener (loads CSV on startup)
    │   └── servlet/
...
- **Apache NetBeans 12+** with the *Java EE / Web* and *Maven* plugin packs enabled
- A servlet container that supports `javax.servlet` (Servlet 4.0):
  - **Apache Tomcat 9.x** (recommended)
  - **Eclipse GlassFish 5** or **Payara 5**

> The project uses `javax.servlet` (Java EE 8 / Servlet 4.0), so it needs
> **Tomcat 9.x** — *not* Tomcat 10+, because Tomcat 10 switched to `jakarta.servlet`.

### Steps
1. **Open NetBeans → File → Open Project** and pick the `AlgoPOS` folder.
   NetBeans will recognise the `pom.xml` and import it as a Maven Web Application.
...
4. NetBeans launches the server, deploys the WAR, and opens
   `http://localhost:8080/AlgoPOS/` in your browser.

### Running from the command line — *no Tomcat install required*
The project bundles an embedded **Jetty 9.4** so you can run it standalone:

```bash
mvn jetty:run
# App is then available at http://localhost:5000/algopos/
```
...
```bash
mvn package
# Drop target/AlgoPOS.war into <tomcat-9>/webapps/ and start Tomcat.
```
...
1. **System Initialization** — `AppBootstrap` reads `products.csv` from the classpath into an `ArrayList<Product>`, sorts it by ID, and stores the `InventoryManager` in the `ServletContext`.
...
5. **Reset** — *Confirm & Print Receipt* shows a printable receipt; *New Transaction* clears the cart for the next customer.
...
2. Open the **Inventory Dashboard** and toggle **₱50 Bill** → *Out of Stock*.
...
|------|------|----------------|
| Delph Xander O. Mogro | Leader | Project coordination |
| Gabrielle L. Pagkaliwangan | Developer | Coding |
| Samara Bianca M. Miyamoto | Analyst | Testing & documentation |
